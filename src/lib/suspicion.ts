...
export const getLearningMinTimeMs = (slideCount?: number) => {
  const effectiveSlides =
    slideCount && slideCount > 0
      ? slideCount
      : SUSPICION_THRESHOLDS.minLearningSlides;
  return SUSPICION_THRESHOLDS.minTimeForReadingSlideMs * effectiveSlides;
};
...
export const getSuspicionRequirements = (slideCount?: number): SuspicionRequirement[] => {
  const effectiveSlides =
    slideCount && slideCount > 0
      ? slideCount
      : SUSPICION_THRESHOLDS.minLearningSlides;
  const learningMinMs = SUSPICION_THRESHOLDS.minTimeForReadingSlideMs * effectiveSlides;

  return [
    {
      id: 'demographics',
      label: `Demographics page time >= ${formatSeconds(SUSPICION_THRESHOLDS.minTimeForDemographicsMs)}s`,
    },
    {
      id: 'pretest',
      label: `Pre-test page time >= ${formatSeconds(SUSPICION_THRESHOLDS.minTimeForPretestMs)}s`,
    },
    {
      id: 'posttest',
      label: `Post-test page time >= ${formatSeconds(SUSPICION_THRESHOLDS.minTimeForPosttestMs)}s`,
    },
    {
      id: 'learning',
      label: `Learning page time >= ${formatSeconds(learningMinMs)}s`,
      details: `Based on ${effectiveSlides} slides x ${formatSeconds(SUSPICION_THRESHOLDS.minTimeForReadingSlideMs)}s minimum.`,
    },
    {
      id: 'slide-view',
      label: `Average slide view time >= ${formatSeconds(SUSPICION_THRESHOLDS.minTimeForReadingSlideMs)}s`,
    },
    {
      id: 'fast-answers',
      label: `Fast answers (<${formatSeconds(SUSPICION_THRESHOLDS.minTimePerQuestionMs)}s) < ${Math.round(SUSPICION_THRESHOLDS.maxFastAnswerRatio * 100)}%`,
    },
    {
      id: 'avg-answer',
      label: `Average answer time >= ${formatSeconds(SUSPICION_THRESHOLDS.minAverageAnswerTimeMs)}s`,
    },
  ];
};

export const SUSPICION_REQUIREMENTS: SuspicionRequirement[] = getSuspicionRequirements();
